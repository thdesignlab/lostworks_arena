...
public class FlickDetector : MonoBehaviour,IGestureDetector
{
	...
	/// </summary>
	[Range (1f, 60f)]
	public int LevelingFrameCount = 2;
	[Range (1f, 100000f)]
	public float DetectAcceleration = 50f;
	[Range (1f, 10000f)]
	public float DefeatSpeed = 50f;
	[Range (0f, 1000f)]
	public float MinFlickDistance = 10f;
	/// <summary>
	/// 指を離さなくても連続で検知するか
	/// </summary>
	...
	{
		if (!(currentInput.IsDown || currentInput.IsDrag || currentInput.IsUp)) return;
		...
		if (this.pastInputs.Count == 1) {
			...
			if (lastInput.IsFlicking) {
                ...
                if (currentInput.SpeedVector.magnitude > this.DefeatSpeed) {
                    ...
                    //Debug.Log("SpeedVector >> " + currentInput.SpeedVector.magnitude + " (" + DefeatSpeed + ")");
                    //フリック中止
                ...
                //フリック開始判定
                //Debug.Log("AccelerationVector >> " + currentInput.AccelerationVector.magnitude + " (" + DetectAcceleration + ")");
